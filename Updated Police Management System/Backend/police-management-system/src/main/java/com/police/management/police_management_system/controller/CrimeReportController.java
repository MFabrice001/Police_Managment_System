...
public class CrimeReportController {
    ...
    public ResponseEntity<CrimeReport> getCrimeById(@PathVariable Long id) {
        Optional<CrimeReport> crime = crimeReportService.getCrimeById(id);
        if (crime.isPresent()) {
            logger.info("Fetched crime report with ID: {}", id);
            return new ResponseEntity<>(crime.get(), HttpStatus.OK);
        } else {
    ...
    public ResponseEntity<CrimeReport> updateCrime(@PathVariable Long id, @RequestBody CrimeReport crimeReport) {
        Optional<CrimeReport> updatedCrime = crimeReportService.updateCrimeReport(id, crimeReport);
        if (updatedCrime.isPresent()) {
            logger.info("Updated crime report with ID: {}", id);
            return new ResponseEntity<>(updatedCrime.get(), HttpStatus.OK);
        } else {
    ...
    public ResponseEntity<Void> deleteCrime(@PathVariable Long id) {
        if (crimeReportService.deleteCrimeReport(id)) {
